/**
 * Symbol Filter — Crypto Futures Only
 * Filter tokenized stocks via areaSymbol field dari Bitget API
 */
...
async function fetchAndCache() {
  try {
    ...
    if (data.code !== '00000' || !Array.isArray(data.data)) throw new Error(`API error: ${data.msg}`);

    const cryptoSet = new Set();
    let totalRwa    = 0;

    for (const s of data.data) {
      if (s.areaSymbol === 'no') cryptoSet.add(s.symbol);
      else totalRwa++;
    }

    _cryptoSymbols = cryptoSet;
    _lastFetch     = Date.now();
    log('symbol_filter', `✅ Cache updated: ${cryptoSet.size} crypto, ${totalRwa} non-crypto difilter`);
    return true;
  } catch (err) {
    log('symbol_filter', `⚠ Gagal fetch symbols: ${err.message}`);
    return false;
  }
}
...
export async function initSymbolFilter() {
  log('symbol_filter', 'Memuat daftar crypto spot dari Bitget...');
  await fetchAndCache();
}
...
export async function filterCryptoOnly(tickers) {
  ...
  if (_cryptoSymbols.size === 0) {
    log('symbol_filter', '⚠ Cache kosong, semua ticker dikembalikan');
    return tickers;
  }
  const before   = tickers.length;
  const filtered = tickers.filter(t => _cryptoSymbols.has(t.symbol));
  const removed  = before - filtered.length;
  if (removed > 0) log('symbol_filter', `Filter: ${before} → ${filtered.length} ticker (${removed} non-crypto dibuang)`);
  return filtered;
}
