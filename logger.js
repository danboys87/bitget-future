...
export function logTrade(action) {
  const ts   = new Date().toISOString();
  const side = action.side === 'open_long'  ? '🟢 LONG'
    : action.side === 'open_short' ? '🔴 SHORT'
    : action.side === 'close_long' ? '🔒 CLOSE LONG'
    : '🔒 CLOSE SHORT';
  fs.appendFileSync(
    path.join(LOG_DIR, `trades-${ts.slice(0, 10)}.jsonl`),
    JSON.stringify({ ts, ...action }) + '\n'
  );
  log('trade', `${side} ${action.symbol} | qty=${action.qty} | price=${action.price} | reason=${action.reason || '-'}`);
}
